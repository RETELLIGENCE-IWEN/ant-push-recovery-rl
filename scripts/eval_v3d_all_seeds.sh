#!/usr/bin/env bash
# Evaluate v3d final models for all 3 seeds against well-trained criteria.
# Run from inside the docker container with /workspace as cwd.
...
for s in "${SEEDS[@]}"; do
  MODEL="runs/well_trained_v3d_seed${s}_1500k/models/final_model.zip"
  OUT="reports/metrics_well_trained_v3d_seed${s}_clean"
  if [[ ! -f "$MODEL" ]]; then
...
for seed in [42, 123, 7]:
    p = Path(f"reports/metrics_well_trained_v3d_seed{seed}_clean/summary.json")
    if not p.exists():
        continue
