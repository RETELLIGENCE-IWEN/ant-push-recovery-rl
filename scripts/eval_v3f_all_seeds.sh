#!/usr/bin/env bash
# Evaluate v3f final models for all 3 seeds against well-trained criteria.
...
for s in "${SEEDS[@]}"; do
  MODEL="runs/well_trained_v3f_seed${s}_500k/models/final_model.zip"
  OUT="reports/metrics_well_trained_v3f_seed${s}_clean"
  if [[ ! -f "$MODEL" ]]; then
    echo "[skip] model missing: $MODEL"
    continue
  fi
  echo "=== eval v3f seed=${s} ==="
...
for seed in [42, 123, 7]:
    p = Path(f"reports/metrics_well_trained_v3f_seed{seed}_clean/summary.json")
    if not p.exists():
        continue
