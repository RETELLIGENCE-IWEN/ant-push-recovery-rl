#!/usr/bin/env bash
# Evaluate v3h_s2 final models for all 3 seeds against well-trained criteria.
...
for s in "${SEEDS[@]}"; do
  MODEL="runs/well_trained_v3h_s2_seed${s}_1000k/models/final_model.zip"
  OUT="reports/metrics_well_trained_v3h_s2_seed${s}_clean"
  if [[ ! -f "$MODEL" ]]; then
    echo "[skip] model missing: $MODEL"
    continue
  fi
  echo "=== eval v3h_s2 seed=${s} ==="
...
for seed in [42, 123, 7]:
    p = Path(f"reports/metrics_well_trained_v3h_s2_seed{seed}_clean/summary.json")
    if not p.exists():
        continue
